...
#senderid -int , id -int , carrierid -int, carryingprice - int , packagevalue -int, packageid int
#returns carry.creditcards int , return sender.creditcards int 
#Author: Mario M. Wadiee

  def withdraw  
    @id = params[:id] #cookies[:user_id]
    @cid = params[:cid]
    @pack = Packages.find(:all,:conditions => ["senders_id = ? AND carriers_id = ?",@id,@cid]).last()
    @sender = Users.find(:all,:conditions => ["id = ?",params[:id]]).first()
  ...
  #userId-int , carrierId-int , packagevalue - int , crrying price - int  
  #Returns creditcard int 
  #Author: Mario M. Wadiee
